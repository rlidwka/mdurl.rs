...
/// ```rust
/// use mdurl::AsciiSet;
/// use mdurl::percent_encode;
///
/// const SAFE_SET : AsciiSet = AsciiSet::from(";/?:@&=+$,-_.!~*'()#");
/// assert_eq!(percent_encode("[hello]", SAFE_SET, true), "%5Bhello%5D");
/// ```
pub fn percent_encode(string: &str, exclude: AsciiSet, keep_escaped: bool) -> Cow<str> {
    ...
    Cow::Owned(String::from_utf8(result).unwrap())
}
...
mod tests {
    ...
    fn should_encode_percent() {
        assert_eq!(percent_encode("%%%", SET, true), "%25%25%25");
    }
    ...
    fn should_not_encode_parts_of_an_url() {
        assert_eq!(percent_encode("?#", SET, true), "?#");
    }

    #[test]
    fn should_encode_square_brackets_commonmark_tests() {
        assert_eq!(percent_encode("[]^", SET, true), "%5B%5D%5E");
    }

    #[test]
    fn should_encode_spaces() {
        assert_eq!(percent_encode("my url", SET, true), "my%20url");
    }

    #[test]
    fn should_encode_unicode() {
        assert_eq!(percent_encode("φου", SET, true), "%CF%86%CE%BF%CF%85");
    }

    #[test]
    fn should_encode_percent_if_it_doesnt_start_a_valid_escape_seq() {
        assert_eq!(percent_encode("%FG", SET, true), "%25FG");
    }

    #[test]
    fn should_preserve_non_utf8_encoded_characters() {
        assert_eq!(percent_encode("%00%FF", SET, true), "%00%FF");
    }

    #[test]
    fn should_encode_characters_on_the_cache_borders() {
        // protects against off-by-one in cache implementation
        assert_eq!(percent_encode("\x00\x7F\u{80}", SET, true), "%00%7F%C2%80");
    }

    #[test]
    fn arguments_encode_string_unescapedset() {
        assert_eq!(percent_encode("!@#$", AsciiSet::from("@$"), true), "%21@%23$");
    }

    #[test]
    fn arguments_keepescaped_true() {
        assert_eq!(percent_encode("%20%2G", SET, true), "%20%252G");
    }

    #[test]
    fn arguments_keepescaped_false() {
        assert_eq!(percent_encode("%20%2G", SET, false), "%2520%252G");
    }
}
